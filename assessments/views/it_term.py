...
from assessments.models import ItTeacherSemester, AssessDepart, Semester, TermType, ItTeacherMidAssess, ItTeacherFinalAssess
from accounts.models import UserInfo, Subject


class AssessModelForm(BootStrapModelForm):
    class Meta:
        model = ItTeacherSemester
        # 字段，所有字段
        fields = '__all__'


def it_term_list(request):
    # 获取所有可选数据
    ...
    # 应用查询条件
    queryset = ItTeacherSemester.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'it_term_list.html', content)


def it_term_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    ItTeacherSemester.objects.filter(id=nid).delete()
    return redirect('assessments:it_term_list')


def it_term_edit(request, pk):
    instance = get_object_or_404(ItTeacherSemester, pk=pk)
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            # 查找对应的期中成绩
            mid_assess = ItTeacherMidAssess.objects.filter(
                teacher=teacher, 
                semester=semester
            ).first()
            
            # 查找对应的期末成绩
            final_assess = ItTeacherFinalAssess.objects.filter(
                teacher=teacher, 
                semester=semester
            ).first()
...
def it_term_import(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            mid_assess_map = {}
            for mid in ItTeacherMidAssess.objects.all():
                key = (mid.teacher_id, mid.semester_id)
                mid_assess_map[key] = mid
                
            final_assess_map = {}
            for final in ItTeacherFinalAssess.objects.all():
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    obj, created = ItTeacherSemester.objects.get_or_create(
                        teacher=teacher,
                        semester=semester,
                        term_type=term_type,
                        defaults=defaults
...
def it_term_export(request):
    ...
    # 获取数据
    queryset = ItTeacherSemester.objects.filter(query).select_related(
...
def it_term_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = ItTeacherSemester.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except ItTeacherSemester.DoesNotExist:
                        not_found_count += 1
