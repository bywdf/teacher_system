...
from assessments.models import GroupLeaderSemester, AssessDepart, Semester, TermType, GroupLeaderMidAssess, GroupLeaderFinalAssess
from accounts.models import UserInfo, Subject


class AssessModelForm(BootStrapModelForm):
    class Meta:
        model = GroupLeaderSemester
        # 字段，所有字段
        fields = '__all__'


def groupleader_term_list(request):
    # 获取所有可选数据
    ...
    # 应用查询条件
    queryset = GroupLeaderSemester.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'groupleader_term_list.html', content)


def groupleader_term_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    GroupLeaderSemester.objects.filter(id=nid).delete()
    return redirect('assessments:groupleader_term_list')


def groupleader_term_edit(request, pk):
    instance = get_object_or_404(GroupLeaderSemester, pk=pk)
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            # 查找对应的期中成绩
            mid_assess = GroupLeaderMidAssess.objects.filter(
                teacher=teacher, 
                semester=semester
            ).first()
            
            # 查找对应的期末成绩
            final_assess = GroupLeaderFinalAssess.objects.filter(
                teacher=teacher, 
                semester=semester
            ).first()
...
def groupleader_term_add(request):
...
# 下面是批量导入需要的功能
@transaction.atomic
def groupleader_term_import(request):
    """批量导入考核成绩并自动关联期中期末成绩"""
    if request.method == "POST":
        ...
        try:
            ...
            mid_assess_map = {}
            for mid in GroupLeaderMidAssess.objects.all():
                key = (mid.teacher_id, mid.semester_id)
                mid_assess_map[key] = mid
                
            final_assess_map = {}
            for final in GroupLeaderFinalAssess.objects.all():
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    obj, created = GroupLeaderSemester.objects.get_or_create(
                        teacher=teacher,
                        semester=semester,
                        term_type=term_type,
                        defaults=defaults
...
def groupleader_term_export(request):
    """导出教师学期考核数据"""
    # 获取筛选参数
    semester_id = request.GET.get('semester')
    term_type_id = request.GET.get('term_type')
    ...
    # 获取数据
    queryset = GroupLeaderSemester.objects.filter(query).select_related(
...
def groupleader_term_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = GroupLeaderSemester.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except GroupLeaderSemester.DoesNotExist:
                        not_found_count += 1
