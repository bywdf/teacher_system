...
from assessments.models import ItTeacherFinalAssess, AssessDepart, Semester, TermType
from accounts.models import UserInfo, Subject


class AssessModelForm(BootStrapModelForm):
    class Meta:
        model = ItTeacherFinalAssess
        # 字段，所有字段
        fields = '__all__'


def it_end_list(request):
    # 获取所有可选数据
    ...
    # 应用查询条件
    queryset = ItTeacherFinalAssess.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'it_end_list.html', content)


def it_end_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    ItTeacherFinalAssess.objects.filter(id=nid).delete()
    return redirect('assessments:it_end_list')


def it_end_edit(request, pk):
    # 获取要编辑的对象，若不存在则返回404
    instance = get_object_or_404(ItTeacherFinalAssess, pk=pk)
    # 创建表单实例，绑定现有数据
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            form.save()
            return redirect('assessments:it_end_list')  # 重定向到列表页
    ...
    context = {
        ...
        'show_video_hours':True,
        'show_teach_book':True,
        'show_network_hours':True, 
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    defaults={
                            ...
                            'video_hours': safe_float(row[7]),
                            'network_hours': safe_float(row[8]),
                    ...
                    obj, created = ItTeacherFinalAssess.objects.get_or_create(
                        teacher=teacher,
...
def it_end_export(request):
    ...
    # 获取数据
    queryset = ItTeacherFinalAssess.objects.filter(query).select_related(
    ...
    headers = [
        ...
        '教师学科', '出勤成绩', '课堂节数', '录像扫描校报等', '网络维修等', '额外工作折算',
        '总工作量节数', '工作量成绩', '监考得分', '常规教学薄成绩',
        '总成绩', '名次', '备注', 
    ]
...
def it_end_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = ItTeacherFinalAssess.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except ItTeacherFinalAssess.DoesNotExist:
                        not_found_count += 1
