...
from assessments.models import  ArtTeacherSemesterAssess, AssessDepart, Semester, TermType, ArtTeacherMidAssess, ArtTeacherFinalAssess
from accounts.models import UserInfo, Subject


class AssessModelForm(BootStrapModelForm):
    class Meta:
        model = ArtTeacherSemesterAssess
        # 字段，所有字段
        fields = '__all__'


def art_term_list(request):
    # 获取所有可选数据
    ...
    # 应用查询条件
    queryset = ArtTeacherSemesterAssess.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'art_term_list.html', content)


def art_term_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    ArtTeacherSemesterAssess.objects.filter(id=nid).delete()
    return redirect('assessments:art_term_list')


def art_term_edit(request, pk):
    instance = get_object_or_404(ArtTeacherSemesterAssess, pk=pk)
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            # 查找对应的期中成绩
            mid_assess = ArtTeacherMidAssess.objects.filter(
                teacher=teacher, 
                semester=semester
            ).first()
            
            # 查找对应的期末成绩
            final_assess = ArtTeacherFinalAssess.objects.filter(
                teacher=teacher, 
                semester=semester
            ).first()
...
def art_term_import(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            mid_assess_map = {}
            for mid in ArtTeacherMidAssess.objects.all():
                key = (mid.teacher_id, mid.semester_id)
                mid_assess_map[key] = mid
                
            final_assess_map = {}
            for final in ArtTeacherFinalAssess.objects.all():
...
def art_term_export(request):
    ...
    # 获取数据
    queryset = ArtTeacherSemesterAssess.objects.filter(query).select_related(
...
def art_term_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = ArtTeacherSemesterAssess.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except ArtTeacherSemesterAssess.DoesNotExist:
                        not_found_count += 1
