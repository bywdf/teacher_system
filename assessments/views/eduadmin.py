...
from assessments.models import EduAdmin, AssessDepart, Semester, TermType
...
def eduadmin_list(request):
    ...
    # 应用查询条件
    queryset = EduAdmin.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'eduadmin_list.html', content)


def eduadmin_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    EduAdmin.objects.filter(id=nid).delete()
    return redirect('assessments:eduadmin_list')


def eduadmin_edit(request, pk):
    # 获取要编辑的对象，若不存在则返回404
    instance = get_object_or_404(EduAdmin, pk=pk)
    # 创建表单实例，绑定现有数据
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            form.save()
            return redirect('assessments:eduadmin_list')  # 重定向到列表页
    ...
    context = {
        ...
        'show_attend_workload':True,
    }
    return render(request, 'assess_change.html', context)


def eduadmin_add(request):
    """添加"""
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    teacher_name = row[5]  # 注意其它考核姓名在row[4]，教务员和行政后勤的姓名列前面加了一列“科室”
                    if not teacher_name:
                    ...
                    defaults={
                            ...
                            'grade_depart': row[4],
                            'attend_score': safe_float(row[6]),
                            'attend_workload': safe_float(row[7]),
                            'democratic_score': safe_float(row[8]),
                            'remark': row[9] or '',
                        }
                    
                    # 创建或更新记录
                    obj, created = EduAdmin.objects.get_or_create(
                        teacher=teacher,
...
def eduadmin_export(request):
    ...
    # 获取数据
    queryset =EduAdmin.objects.filter(query).select_related(
...
def eduadmin_update_rank(request):
