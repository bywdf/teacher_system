...
from assessments.models import GroupLeaderMidAssess, AssessDepart, Semester, TermType
from accounts.models import UserInfo, Subject


class MidAssessModelForm(BootStrapModelForm):
    class Meta:
        model = GroupLeaderMidAssess
        # 字段，所有字段
        fields = '__all__'


def groupleader_mid_list(request):
    # 获取所有可选数据
    ...
    # 应用查询条件
    queryset = GroupLeaderMidAssess.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'groupleader_mid_list.html', content)


def groupleader_mid_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    GroupLeaderMidAssess.objects.filter(id=nid).delete()
    return redirect('assessments:groupleader_mid_list')


def groupleader_mid_edit(request, pk):
    # 获取要编辑的对象，若不存在则返回404
    instance = get_object_or_404(GroupLeaderMidAssess, pk=pk)
    # 创建表单实例，绑定现有数据
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            form.save()
            return redirect('assessments:groupleader_mid_list')  # 重定向到列表页
    ...
    context = {
        ...
        'title': '考核记录',
        'instance': instance,
        'show_plan_summary_score':True,
        'show_teach_level_score':True,
    }
    return render(request, 'assess_change.html', context)


def groupleader_mid_add(request):
    """添加"""
...
# 下面是批量导入需要的功能
@transaction.atomic
def groupleader_mid_import(request):
    """批量导入考核成绩"""
    if request.method == "POST":
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    defaults={
                            ...
                            'plan_summary_score': safe_float(row[5]),
                            'teach_level_score': safe_float(row[6]),
                        }
                    
                    # 创建或更新记录
                    obj, created = GroupLeaderMidAssess.objects.get_or_create(
                        teacher=teacher,
...
def groupleader_mid_export(request):
    """导出教师期中考核数据"""
    # 获取筛选参数
    semester_id = request.GET.get('semester')
    term_type_id = request.GET.get('term_type')
    ...
    # 获取数据
    queryset =GroupLeaderMidAssess.objects.filter(query).select_related(
    ...
    # 设置表头
    headers = [
        '序号', '学期', '考核类型', '考核时间', '考核部门', '组长',
        '教研组', '计划', '教学水平/教师常规薄本评价', 
        '总成绩', '名次',
    ]
    ...
    for row_num, obj in enumerate(queryset, start=2):
        data = [
            ...
            obj.plan_summary_score,
            obj.teach_level_score,    
            obj.total_score,
            obj.rank,
        ]
...
def groupleader_mid_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = GroupLeaderMidAssess.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except GroupLeaderMidAssess.DoesNotExist:
                        not_found_count += 1
