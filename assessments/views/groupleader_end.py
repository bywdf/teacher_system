...
class AssessModelForm(BootStrapModelForm):
    class Meta:
        model = GroupLeaderFinalAssess
        # 字段，所有字段
        fields = '__all__'


def groupleader_end_list(request):
    # 获取所有可选数据
    ...
    # 应用查询条件
    queryset = GroupLeaderFinalAssess.objects.filter(
        query).order_by('id')
...
def groupleader_end_delete(request):
    ...
    GroupLeaderFinalAssess.objects.filter(id=nid).delete()
...
def groupleader_end_edit(request, pk):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            form.save()
            return redirect('assessments:groupleader_end_list')  # 重定向到列表页
...
# 下面是批量导入需要的功能
@transaction.atomic
def groupleader_end_import(request):
    """批量导入考核成绩"""
    if request.method == "POST":
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 创建或更新记录
                    obj, created = GroupLeaderFinalAssess.objects.get_or_create(
                        teacher=teacher,
...
def groupleader_end_export(request):
    """导出教师考核数据"""
    # 获取筛选参数
    semester_id = request.GET.get('semester')
    term_type_id = request.GET.get('term_type')
    ...
    # 获取数据
    queryset =GroupLeaderFinalAssess.objects.filter(query).select_related(
...
def groupleader_end_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = GroupLeaderFinalAssess.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except GroupLeaderFinalAssess.DoesNotExist:
                        not_found_count += 1
