...
from assessments.models import DeputyHeadTeacher, AssessDepart, Semester, TermType
...
def deputy_list(request):
    ...
    # 应用查询条件
    queryset = DeputyHeadTeacher.objects.filter(
        query).order_by('id')
    ...
    return render(request, 'deputy_list.html', content)


@superuser_required
def deputy_delete(request):
    """删除"""
    nid = request.GET.get('nid')
    DeputyHeadTeacher.objects.filter(id=nid).delete()
    return redirect('assessments:deputy_list')


@superuser_required
def deputy_edit(request, pk):
    # 获取要编辑的对象，若不存在则返回404
    instance = get_object_or_404(DeputyHeadTeacher, pk=pk)
    # 创建表单实例，绑定现有数据
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            form.save()
            return redirect('assessments:deputy_list')  # 重定向到列表页
    ...
    context = {
        ...
        'show_questionnaire_score': True,
        'show_work_process_score': True,
        'show_work_experience_score': True,
        'show_democratic_score': True,
        'show_edge_student_score': True,
        'show_class_manage_score': True,
    }
    return render(request, 'assess_change.html', context)


@superuser_required
def deputy_add(request):
    """添加"""
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    teacher_name = row[4]  # 注意其它考核姓名在row[4]，教务员和行政后勤的姓名列前面加了一列“科室”
                    if not teacher_name:
                    ...
                    defaults={
                            ...
                            'class_number': row[5],
                            'questionnaire_score': safe_float(row[6]),
                            'work_process_score': safe_float(row[7]),
                            'work_experience_score': safe_float(row[8]),
                            'democratic_score': safe_float(row[9]),
                            'edge_student_score': safe_float(row[10]),
                            'class_score': safe_float(row[11]),
                            'remark': row[12] or '',
                        }
                    
                    # 创建或更新记录
                    obj, created = DeputyHeadTeacher.objects.get_or_create(
                        teacher=teacher,
...
def deputy_export(request):
    ...
    # 获取数据
    queryset =DeputyHeadTeacher.objects.filter(query).select_related(
    ...
    # 设置表头
    headers = [
        '序号', '学期', '考核类型', '考核时间', '考核部门', '姓名', ' 学科',
        '任职班级', '问卷评价5%', '工作过程评价20%', '工作心得20%', '民主评议20%', '边缘生帮扶25%', '班级综考成绩25%'
        '总成绩', '名次', '备注',
    ]
    ...
    for row_num, obj in enumerate(queryset, start=2):
        data = [
            ...
            obj.class_number,
            obj.questionnaire_score,
            obj.work_process_score,
            obj.work_experience_score,
            obj.democratic_score,
            obj.edge_student_score,
            obj.class_score,
            obj.total_score,
            obj.rank,
            obj.remark,
...
@superuser_required
def deputy_update_rank(request):
        ...
        try:
            ...
            for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                try:
                    ...
                    # 查找并更新记录
                    try:
                        assess = DeputyHeadTeacher.objects.get(
                            teacher=teacher,
                            semester=semester,
                            term_type=term_type
                        )
                    ...
                    except DeputyHeadTeacher.DoesNotExist:
                        not_found_count += 1
